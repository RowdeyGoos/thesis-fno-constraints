...
generate_poisson() {
    ...
    "$PYTHON_BIN" utils/gen_data_poisson.py \
        --ntrain="$ntrain" \
        --nval="$nval" \
        --ntest="$ntest" \
        --ng="$ng" \
    ...
    generate_mixed_copies \
        "Poisson data (${label})" \
        "$datapath" \
        "utils/convert_poisson_to_mixed_format.py" \
...
generate_advdiff() {
    ...
    "$PYTHON_BIN" utils/gen_data_advdiff.py \
        --ntrain="$ntrain" \
        --nval="$nval" \
        --ntest="$ntest" \
        --ng="$ng" \
    ...
    generate_mixed_copies \
        "AdvDiff data (${label})" \
        "$datapath" \
        "utils/convert_ad_to_mixed_format.py" \
        "$train_file" "$val_file" "$test_file"
}
...
generate_helmholtz() {
    ...
    "$PYTHON_BIN" utils/gen_data_helmholtz.py \
        --ntrain="$ntrain" \
        --nval="$nval" \
        --ntest="$ntest" \
        --ng="$ng" \
    ...
    generate_mixed_copies \
        ...
        "$datapath" \
        "utils/convert_helmholtz_to_mixed_format.py" \
        "$train_file" "$val_file" "$test_file"
}
