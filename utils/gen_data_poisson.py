...
if __name__ == '__main__':
    ...
    train_path = os.path.join(args.datapath, "_train_k{}_{}_32k.h5".format(params.e1, params.e2))
    val_path = os.path.join(args.datapath, "_val_k{}_{}_4k.h5".format(params.e1, params.e2))
    test_path = os.path.join(args.datapath, "_test_k{}_{}_4k.h5".format(params.e1, params.e2))

    train_file, train_fields_ds, train_tensor_ds = create_hdf5_datasets(
        train_path, ntrain, x.shape[0], y.shape[0], 3, args.h5_chunk_samples
    )
    val_file, val_fields_ds, val_tensor_ds = create_hdf5_datasets(
        val_path, nval, x.shape[0], y.shape[0], 3, args.h5_chunk_samples
    )
    test_file, test_fields_ds, test_tensor_ds = create_hdf5_datasets(
        test_path, ntest, x.shape[0], y.shape[0], 3, args.h5_chunk_samples
    )
    ...
    try:
        ...
        for idx, vf in enumerate(vfs):
            ...
            for _ in range(nex):
                ...
                train_tensor_ds[sim_train] = k * params.diff_coef_scale
                sim_train += 1
            ...
            for _ in range(nex):
                ...
                val_tensor_ds[sim_val] = k * params.diff_coef_scale
                sim_val += 1
            ...
            for _ in range(nex):
                ...
                test_tensor_ds[sim_test] = k * params.diff_coef_scale
                sim_test += 1
