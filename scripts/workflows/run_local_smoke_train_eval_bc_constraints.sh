...
"$PYTHON_BIN" scripts/data/gen_data_poisson_bc.py \
...
"$PYTHON_BIN" scripts/data/gen_data_advdiff_bc.py \
  --ntrain "$NTRAIN_PER_SYSTEM" --nval "$NVAL_PER_SYSTEM" --ntest "$NTEST_PER_SYSTEM" \
  --n "$GRID_N" --ng "$NG" --sparse --datapath "$ADVDIFF_DIR" --adr1 0.2 --adr2 1.0

"$PYTHON_BIN" scripts/data/gen_data_helmholtz_bc.py \
  --ntrain "$NTRAIN_PER_SYSTEM" --nval "$NVAL_PER_SYSTEM" --ntest "$NTEST_PER_SYSTEM" \
...
"$PYTHON_BIN" scripts/data/check_bc_dataset_sanity.py --input "$POISSON_DIR" --glob "*.h5"
"$PYTHON_BIN" scripts/data/check_bc_dataset_sanity.py --input "$ADVDIFF_DIR" --glob "*.h5"
"$PYTHON_BIN" scripts/data/check_bc_dataset_sanity.py --input "$HELMHOLTZ_DIR" --glob "*.h5"

echo "Building mixed BC datasets (train/val/test)..."
"$PYTHON_BIN" scripts/data/create_mixed_dataset.py \
...
"$PYTHON_BIN" scripts/data/check_bc_dataset_sanity.py --input "$MIXED_DIR" --glob "*_mixed_bc_smoke.h5"
