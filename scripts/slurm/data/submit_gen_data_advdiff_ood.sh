#!/usr/bin/env bash
#SBATCH --job-name=data-advdiff-ood
...
# Default OOD bins beyond the AdvDiff source/pretrain range adr in [0.2, 1.0].
RANGES_CSV="${OOD_RANGES:-1.0:1.2,1.2:1.4,1.4:1.6,1.6:1.8}"
IFS=',' read -r -a RANGES <<< "${RANGES_CSV}"

TASK_ID="${SLURM_ARRAY_TASK_ID:-0}"
if [ "${TASK_ID}" -lt 0 ] || [ "${TASK_ID}" -ge "${#RANGES[@]}" ]; then
    echo "Error: task index ${TASK_ID} is out of range for ${#RANGES[@]} AdvDiff OOD bins."
...
if [ -z "${RANGE_MIN:-}" ] || [ -z "${RANGE_MAX:-}" ]; then
    echo "Error: invalid AdvDiff range '${RANGES[${TASK_ID}]}' (expected min:max)."
    exit 1
fi

export DATASET="advdiff"
export RANGE_SET="transfer"
export DATA_ROOT="${DATA_ROOT:-data}"
export GENERATE_MIXED_FORMAT="${GENERATE_MIXED_FORMAT:-1}"
export TRANSFER_ADVDIFF_ADR1="${RANGE_MIN}"
export TRANSFER_ADVDIFF_ADR2="${RANGE_MAX}"

echo "=========================================="
echo "AdvDiff OOD data generation"
echo "Task ID:       ${TASK_ID}"
echo "Range list:    ${RANGES_CSV}"
echo "Selected adr:  [${TRANSFER_ADVDIFF_ADR1}, ${TRANSFER_ADVDIFF_ADR2}]"
