...
#   bash scripts/maintenance/update_mixed_checkpoint_path.sh 12345

if [ $# -lt 1 ]; then
    echo "Usage: bash scripts/maintenance/update_mixed_checkpoint_path.sh <mixed_pretrain_job_id>"
    echo ""
    ...
    echo ""
    echo "Example:"
    echo "  bash scripts/maintenance/update_mixed_checkpoint_path.sh 12345"
    echo "  PRETRAIN_CONFIG_NAME=mixed-scale-all-constraints-al-hard PRETRAIN_RUN_PREFIX=pretrain-mixed-al-hard bash scripts/maintenance/update_mixed_checkpoint_path.sh 12345"
    exit 1
fi
